...
<div class="fieldcontain ${hasErrors(bean: applicationInstance, field: 'status', 'error')} required">
	...
	<g:select id="status" name="status.id" from="${placements.Status.list()}" optionKey="id" optionValue="description" required="" value="${applicationInstance?.status?.id}" class="many-to-one"/>
</div>
